...
document.getElementById("loginForm").addEventListener("submit", function (event) {
    ...
    if (email && password) {
        ...
        if (password.length < minLength) {
            passwordRequirements.textContent = "Password should be at least 8 characters long";
        } else if (!regex.test(password)) {
            passwordRequirements.textContent = "Password must contain at least one uppercase letter, lowercase letter, and one digit";
        } else {
